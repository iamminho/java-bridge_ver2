package bridge.domain;

import bridge.domain.dto.BridgeMap;
import bridge.domain.dto.Moving;
import bridge.domain.dto.MovingResult;

/**
 * 다리 건너기 게임을 관리하는 클래스
 */
public class BridgeGame {
    private final String RETRY_COMMAND = "R";
    private Bridge bridge;
    private BridgeMap bridgeMap;

    public BridgeGame(Bridge bridge, BridgeMap bridgeMap) {
        this.bridge = bridge;
        this.bridgeMap = bridgeMap;
    }

    /**
     * 사용자가 칸을 이동할 때 사용하는 메서드
     * <p>
     * 이동을 위해 필요한 메서드의 반환 타입(return type), 인자(parameter)는 자유롭게 추가하거나 변경할 수 있다.
     */
    public BridgeMap move(Moving moving) {
        bridgeMap.addResult(getResult(moving));
        return this.bridgeMap;
    }

    private MovingResult getResult(Moving moving) {
        return bridge.getMovingResult(moving);
    }

    /**
     * 사용자가 게임을 다시 시도할 때 사용하는 메서드
     * <p>
     * 재시작을 위해 필요한 메서드의 반환 타입(return type), 인자(parameter)는 자유롭게 추가하거나 변경할 수 있다.
     */
    public boolean retry(String command) {
        if (command.equals(RETRY_COMMAND)) {
            bridgeMap.resetMap();
            bridge.resetLocation();
            return true;
        }
        return false;
    }
}
